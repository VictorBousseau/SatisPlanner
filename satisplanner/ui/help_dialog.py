...
def gestures() -> tuple[tuple[str, str], ...]:
    """(gesture, what it does), in the language in force.

    The canvas is the part of this application nobody can guess, so it comes first
    and gets the room. Built on demand rather than at import, like every other
    table of sentences here: the page is opened long after the language is chosen.
    """
    return (
        (
            _("Glisser une entrée de la palette sur le canvas"),
            _("pose le nœud à l'endroit lâché, aligné sur la grille"),
        ),
        (
            _("Double-clic dans la palette"),
            _("ouvre la fiche de l'objet : recettes, machines, débits, coût en minerai"),
        ),
        (
            _("Dans une fiche, clic sur un ingrédient"),
            _("ouvre sa fiche. Alt+Gauche et Alt+Droite reviennent en arrière et en avant"),
        ),
        (
            _("Dans une fiche, [poser sur le canvas]"),
            _("pose cette recette au centre de la vue"),
        ),
        (
            _("Glisser d'un port de sortie vers un port d'entrée"),
            _("tire une ligne. Le trait est vert si elle peut exister, rouge sinon, "
            "avec la raison en infobulle pendant le tirage"),
        ),
        (
            _("Lâcher n'importe où sur un tampon vide"),
            _("le raccorde : un tampon sans contenu accepte le premier item qui arrive"),
        ),
        (_("Glisser un nœud"), _("le déplace. Un glissement continu vaut une seule annulation")),
        (_("Glisser sur le fond"), _("sélection rectangulaire")),
        (_("Clic milieu maintenu"), _("déplace la vue")),
        (_("Molette"), _("zoom avant et arrière autour du curseur")),
        (
            _("Clic droit sur un nœud"),
            _("fiche de l'objet, ajuster aux intrants, nombre de machines, cadence, "
            "pureté du gisement, extracteur, carburant du générateur, contenu du "
            "tampon, supprimer"),
        ),
        (
            _("Entrée dans la palette"),
            _("pose l'entrée surlignée au centre de la vue"),
        ),
        (
            _("Double-clic sur une valeur d'un nœud"),
            _("l'édite sur place : nombre de machines, cadence, pureté, extracteur, "
        ...
        (
            _("Double-clic sur une ligne"),
            _("change son tier, dans la même liste que le menu contextuel"),
        ),
        (
            _("Copier une sélection"),
            _("emporte les nœuds et les lignes qui les relient entre eux ; une ligne qui "
            "sortait de la sélection n'a plus de quoi s'accrocher et n'est pas copiée"),
        ),
        (
            _("Affichage ▸ Machines déployées"),
            _("dessine une vignette par machine bâtie sur chaque nœud. Clic droit sur un "
            "nœud pour y déroger. Purement visuel : aucun chiffre ne change"),
        ),
        (
            _("Clic droit sur une ligne"),
            _("changer de tier, passer au tier suffisant quand elle sature, supprimer"),
        ),
        (
            _("Clic sur une ligne du tableau"),
            _("sélectionne le nœud sur le canvas, et réciproquement"),
        ),
        (
            _("Clic sur un diagnostic"),
            _("sélectionne et centre le nœud ou la ligne concernée"),
        ),
        (
            _("Raccords ▸ Mode simple / Mode fidèle"),
            _("bascule toute l'usine, en une seule annulation. Vers le fidèle, les arbres "
            "de raccords sont posés ; vers le simple, ils sont dissous. Ce qui change "
            "de débit est dit port par port"),
        ),
        (
            _("Générer ▸ Générer une usine depuis un objectif"),
            _("développe « tant de cet objet par minute » en une usine entière, dans un "
            "onglet neuf. Une recette imposée par objet est retenue d'une fois sur "
            "l'autre ; le rapport dit ce qu'il reste à régler"),
        ),
        (
            _("Palette, section Raccords"),
            _("un répartiteur et un groupeur se posent comme n'importe quel autre nœud, "
            "et leur mode se règle par clic droit ou par la colonne du tableau"),
        ),
        (
            _("Modules ▸ Enregistrer la sélection comme module"),
            _("range le morceau sélectionné dans la bibliothèque, avec ses débits d'entrée "
        ...
        (
            _("Modules ▸ Bibliothèque"),
            _("insérer au centre de la vue, ouvrir dans un onglet pour modifier, démarrer "
            "une usine depuis un module, renommer, décrire, supprimer"),
        ),
    )


def modelling_notes() -> tuple[str, ...]:
    """Rules a user cannot deduce from the interface and will otherwise assume wrongly.

    Each one is a modelling choice, not a limitation to be worked around, and each
    says **why** rather than only what. This is the page somebody reads to decide
    whether to trust the numbers, so the English has to argue as well as the French
    does -- a transposition that reads like a translation would lose the argument.
    """
    return (
        _("La pureté d'un gisement s'applique à <b>tous</b> les extracteurs de ce nœud : "
        ...
        _("<b>Un puits de ressource est deux bâtiments</b>, et un seul des deux consomme. "
        ...
        _("La cadence multiplie le débit à l'identique et l'électricité en loi de "
        ...
        _("<b>Trois machines consomment selon ce qu'elles fabriquent</b>, et non selon "
        ...
        _("<b>Le nombre de machines est ce que vous saisissez</b>, jamais ce que l'outil "
        ...
        _("<b>La centrale nucléaire produit sur une ligne</b>, et c'est le seul générateur "
        ...
        _("<b>Le générateur géothermique ne brûle rien</b> : il se pose sur un geyser, et "
        ...
        _("<b>Un sous-produit sans issue arrête la machine entièrement.</b> Une recette "
        ...
        _("<b>La capacité d'une ligne est une contrainte, pas une remarque.</b> Un "
        ...
        _("<b>Chaque usine est en mode simple ou en mode fidèle</b>, et le réglage est "
        ...
        _("<b>La liste de courses compte les raccords dans les deux modes, et pas de la "
        ...
        _("Un raccord <b>intelligent ou programmable interdit le retour au mode "
        ...
        _("<b>Un port porte une ligne</b> en mode fidèle, et un nœud a autant de ports qu'il a de "
        ...
        _("<b>Le partage est max-min, pas proportionnel.</b> Chaque branche reçoit une "
        ...
        _("<b>Générer une usine</b> développe une cible — « deux cadres modulaires "
        ...
        "immédiatement."),
        _("Ce que le générateur ne peut pas savoir, il le dit : <b>les gisements sont "
        ...
        _("Un répartiteur se règle : <b>standard</b>, <b>intelligent</b> (une branche "
        ...
        _("<b>Filtrer sur un objet ne change rien, ici.</b> Une ligne ne porte qu'un "
        ...
        _("<b>Le brûleur de biomasse n'a pas de chaîne d'alimentation, et ce n'est pas "
        ...
        _("<b>L'azote ne sort du sol que par un puits de ressource</b> : aucune foreuse et "
        ...
        _("<b>Toutes les machines du jeu sont au catalogue</b>, et les 291 recettes qu'elles "
        ...
        _("<b>Une machine à l'arrêt consomme quand même.</b> La production suit le taux "
        ...
        _("Les tampons sont des puits et des sources infinis. L'application dit si les "
        "débits sont tenables et en combien de temps un stock se vide, mais ne simule "
        "pas le temps qui passe."),
        _("Rien ici n'est de la géométrie : ni distance, ni élévation, ni hauteur de "
        "refoulement des pompes."),
        _("Un <b>module inséré est une copie</b>. Il ne garde aucun lien avec sa "
        ...
        _("Les <b>matériaux de construction</b> donnent le coût exact des machines, "
        ...
        _("L'électricité est un <b>compteur, pas une contrainte</b> : consommation et "
        ...
        _("La consommation compte les machines <b>à l'arrêt</b> : elles sont construites et "
...
def help_html(shortcuts: Sequence[tuple[str, str]]) -> str:
    ...
    gesture_rows = "".join(
        f"<tr><td class='key'>{gesture}</td><td>{effect}</td></tr>"
    ...
    canvas_heading = _("Gestes du canvas")
    shortcut_heading = _("Raccourcis")
    model_heading = _("Ce que l'outil modélise, et comment")
    delete_note = _(
        "La touche Suppr efface la sélection, nœuds et lignes confondus. "
        "Tout passe par la pile d'annulation, déplacements compris."
    )
    return f"""
    <style>
      body {{ font-size: 10pt; }}
    ...
    <h2>{canvas_heading}</h2>
    <table>{gesture_rows}</table>
    <h2>{shortcut_heading}</h2>
    <table>{keys}</table>
    <p class="note">{delete_note}</p>
    <h2>{model_heading}</h2>
    <ul>{notes}</ul>
    """
...
class HelpDialog(QDialog):
    ...
    def __init__(self, shortcuts: Sequence[tuple[str, str]], parent: QWidget | None = None) -> None:
        ...
        self.setWindowTitle(_("Gestes et raccourcis"))
        self.resize(680, 640)
